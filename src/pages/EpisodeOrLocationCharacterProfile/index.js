...
import './EpisodeOrLocationCharacterProfile.css';

const EpisodeOrLocationCharacterProfile = () => {
  const params = useParams();
  const { episodeId, locationId, characterId } = params;

  const episodeOrLocationCharacterProfile = useSelector(
    (state) => state.episodeOrLocationCharacterProfile
  );
  const {
    characterImg,
    characterName,
    ...
    characterLocation,
    characterType,
  } = episodeOrLocationCharacterProfile;
  ...
  return (
    <Box>
      ...
      >
        <Navbar />
        {(episodeCharacterListId.includes(characterId) &&
          !characterPageError) ||
        (locationCharacterListId.includes(characterId) &&
          !characterPageError) ||
        ((episodeCharacterListId.includes(characterId) ||
          locationCharacterListId.includes(characterId)) &&
          !characterPageError) ? (
          ...
          >
            {characterPageError || 'Character not found'}
          </Typography>
        )}
      </Box>
    </Box>
...
export default EpisodeOrLocationCharacterProfile;
